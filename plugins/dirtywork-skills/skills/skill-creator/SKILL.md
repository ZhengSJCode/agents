...
```bash
scripts/init_skill.py <skill-name> --path <output-directory> [--template minimal|complete]
```

**Template options:**
- `minimal` (default) - Basic template with essential fields: name, description
- `complete` - Full best-practices template with:
  - Version tracking (semver)
  - License field (SPDX identifier)
  - Complete metadata section:
    - author, category, tags, triggers
    - userInvocable flag
  - allowed-tools array
...
**Example with complete template:**
```bash
scripts/init_skill.py my-awesome-skill --path skills/public --template complete
```
...
**For complete template, also fill:**
- **version**: Semantic versioning (1.0.0)
- **license**: SPDX identifier (Apache-2.0 recommended)
- **metadata.author**: Author name
- **metadata.tags**: Searchability tags
- **metadata.triggers**: Explicit trigger phrases
- **metadata.category**: Organization category
- **allowed-tools**: Tools this skill may use

### Validate Frontmatter

Run the validation script to check frontmatter against best practices:

```bash
scripts/validate_frontmatter.py <skill-path>
```
...
Skills can be distributed through marketplace.json files for team sharing and easy installation.
...
```bash
scripts/generate_marketplace.py <skill-path> --owner <username>
```

**Options:**
- `--owner`: Owner name (defaults to git config or 'your-username')
- `--source-type`: Source type (auto-detected, options: relative, github, url, git-subdir, npm)
- `--repo`: GitHub repo (for github sources)
- `--url`: Git URL (for url/git-subdir sources)

**Example:**
```bash
scripts/generate_marketplace.py skills/my-skill --owner myusername
```
...
```bash
scripts/validate_marketplace.py <path-to-marketplace.json>
```

### Package with Marketplace.json

Include marketplace.json when packaging skills:

```bash
scripts/package_skill.py <skill-path> --include-marketplace --owner <username>
```

The .skill file will include marketplace.json for easy distribution.

---

## Description Testing

A skill's description determines when it triggers. Poor descriptions lead to false positives (triggering when wrong) or false negatives (not triggering when right). The description testing script helps optimize descriptions.

### Test Description

Test a skill description against sample prompts:

```bash
scripts/test_description.py <skill-path> [options]
```

**Options:**
- `--evals <path>`: Path to evals.json file for test prompts
- `--prompts <file>`: Text file with sample prompts (one per line)
- `--interactive`: Interactive mode for testing prompts
- `--output <file>`: Save results to JSON file

**Example with evals.json:**
```bash
scripts/test_description.py skills/my-skill --evals evals/evals.json
```

**Example with prompts file:**
```bash
...
**Interactive mode:**
```bash
scripts/test_description.py skills/my-skill --interactive
```
...
|--------|---------|---------------|
| **Precision** | TP / (TP + FP) | Of all triggers, how many were correct? |
| **Recall** | TP / (TP + FN) | Of all should-trigger prompts, how many matched? |
| **F1 Score** | 2 × (P × R) / (P + R) | Harmonic mean of precision and recall |

**Target ranges:**
- Precision: ≥ 0.8 (minimize false triggers)
- Recall: ≥ 0.5 (catch most relevant requests)
- F1 Score: ≥ 0.65 (balanced performance)

### Improvement Suggestions

The script generates suggestions based on test results:

1. **Description too short** - Add more context about when to use this skill
2. **No explicit trigger phrases** - Add format: `when the user asks to "X", "Y", or "Z"`
3. **Low precision** - Description too broad, add more specific criteria
4. **Low recall** - Description too narrow, add more trigger phrases
5. **No examples** - Add examples of specific user requests that should trigger
...
💡 Suggestions:
   1. Add explicit trigger phrases using format: 'when the user asks to "X"'
   2. Consider adding explicit trigger phrases for better discoverability
```
...
For comprehensive validation of all aspects of a skill, use the master validator:

```bash
scripts/validate_all.py <skill-path> [options]
```
...
- `--fix-frontend`: Auto-fix simple frontmatter issues (add missing name/version/license)
- `--strict`: Treat warnings as errors
- `--json`: Output as JSON

**What it validates:**
1. **Frontmatter** - Required fields, version format, license, metadata
2. **Marketplace.json** - Schema validation (if present)
3. **Skill Structure** - Required files, executable scripts
4. **Description** - Trigger precision and recall (if --test-description)
...
scripts/validate_all.py skills/my-skill --test-description --prompts test_prompts.txt

# Auto-fix simple issues and validate strictly
scripts/validate_all.py skills/my-skill --fix-frontend --strict

# Output as JSON for CI/CD
scripts/validate_all.py skills/my-skill --json
```
...
Skill-creator includes templates for common reference files:

```bash
templates/references/
├── README.md              # Usage guidance
├── techniques.md.template  # Methodologies, algorithms
├── examples.md.template    # Worked examples
├── checklist.md.template   # Pre-flight/validation
...
```bash
# Copy templates to your skill
cp -r templates/references/* skills/my-skill/references/
...
|----------|---------|----------|
| `techniques.md` | Document algorithms and methodologies | You have multiple technical approaches to explain |
| `examples.md` | Provide worked examples | Examples would clutter SKILL.md |
| `checklist.md` | Define quality criteria | You need standardized validation steps |
| `troubleshooting.md` | Document common issues | Users may encounter errors or edge cases |

See `templates/references/README.md` for detailed guidance.
...
```bash
scripts/package_skill.py <path/to/skill-folder> --include-marketplace --owner <username>
```
